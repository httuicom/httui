...
/// "overridden locally" badges. Round-trip is one IPC
/// call instead of two.
#[tauri::command]
...
/// Used by the Open-vault flow before activating it.
#[tauri::command]
...
/// which ones are missing from the local OS keychain. Used by
/// the first-run flow to batch-prompt the user
