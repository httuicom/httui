...
// `Mode` no longer used in production here (moved with `parse_visual`
// to `crate::input::parser::visual` in fase 1 p3c); kept in scope so
// the in-file `mod tests` (`use super::*`) keeps resolving it unchanged.
